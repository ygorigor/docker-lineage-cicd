...
for codename in ${devices//,/ }; do
  if [ -n "$codename" ]; then
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        ( yes||: ) | repo init -u https://github.com/ygorigor/android.git --reference "$MIRROR_DIR" -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
      else
        ( yes||: ) | repo init -u https://github.com/ygorigor/android.git -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
      fi
    else
